...
import 'i_local_file_datasource.dart';

class LocalFileDataSource implements ILocalFileDataSource {
  final DateFormat _timestampFormatter = DateFormat('yyyyMMdd_HHmmss');

  String? _audioFilePath;

  @override
  Future<String> get audoFilePath async {
    if (_audioFilePath == null) {
      final tempDir = await getTemporaryDirectory();
      final filename = 'speech_${_timestampFormatter.format(DateTime.now())}.wav';
  ...
  Future<String> saveTranscriptFile({
    ...
    final audioSource = File(audioFileName);
    final savedAudio = File(
      p.join(sessionDir.path, p.basename(audioFileName)),
    );
  ...
  String _buildHtml(List<TranscriptSegment> segments, String audioPath) {
